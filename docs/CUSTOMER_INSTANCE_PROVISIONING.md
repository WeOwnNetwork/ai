...
| **Backups & retention** | Daily GFS backups (30d/12mo/yearly), client-side encrypted per the Encryption row (`BACKUP_AGE_RECIPIENT` in the site project; identity in operator-tools as `BACKUP_AGE_IDENTITY_<project>` — generated automatically by `deploy-new-site.sh`). Restores stage the private key into droplet tmpfs for the run and shred it. On cancellation: final backup retained **60 days**, then deleted from Spaces; droplet, reserved IP, Infisical project, and the customer's OpenRouter key destroyed/revoked at deprovision. |
