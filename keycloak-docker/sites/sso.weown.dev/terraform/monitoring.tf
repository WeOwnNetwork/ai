...
resource "digitalocean_monitor_alert" "cpu" {
  ...
  description = "sso: CPU usage above ${var.cpu_alert_threshold}% for 5 min"
}
...
resource "digitalocean_monitor_alert" "memory" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.keycloak.id]
  description = "sso: Memory usage above ${var.memory_alert_threshold}% for 5 min"
}
...
resource "digitalocean_monitor_alert" "disk" {
  ...
  enabled     = true
  entities    = [digitalocean_droplet.keycloak.id]
  description = "sso: Disk usage above ${var.disk_alert_threshold}% for 5 min"
}
