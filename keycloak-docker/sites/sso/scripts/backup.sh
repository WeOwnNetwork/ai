#!/usr/bin/env bash
# sso-keycloak - Skinny Backup Script
# Backs up PostgreSQL database, Keycloak data volumes, and configuration.
#
# Usage:
...
BACKUP_NAME="sso-keycloak_backup_$TIMESTAMP"
...
# --- Database dump ---
# Names must match compose.prod.yaml / Infisical (POSTGRES_*), not the legacy DB_*.
echo "==> Dumping PostgreSQL database..."
docker exec -e PGPASSWORD="$POSTGRES_PASSWORD" "${PROJECT_NAME}-db-1" \
  pg_dump -U "${POSTGRES_USER:-keycloak}" -d "${POSTGRES_DB:-keycloak}" \
...
# FAILED run. A warning-and-exit-0 here let the daily cron report success for
# weeks while no object ever reached the bucket.
if [[ "$REMOTE_STORAGE" == "do-spaces" ]]; then
  if [[ -z "${SPACES_ACCESS_KEY:-}" ]] || [[ -z "${SPACES_SECRET_KEY:-}" ]]; then
    ...
    echo "       Refusing to report success without the off-box copy." >&2
    exit 1
  fi
  echo "==> Uploading to DO Spaces (s3://${SPACES_BUCKET}/sso-keycloak/)..."
  export AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY"
  export AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY"
  aws s3 cp "$BACKUP_DIR/${BACKUP_NAME}.tar.gz" \
    "s3://${SPACES_BUCKET}/sso-keycloak/" \
  ...
  REMOTE_BYTES=$(aws s3 ls "s3://${SPACES_BUCKET}/sso-keycloak/${BACKUP_NAME}.tar.gz" \
    --endpoint-url "https://${SPACES_REGION}.digitaloceanspaces.com" \
    | awk '{print $3}' | tail -1)
  ...
  echo "==> Remote backup verified: ${BACKUP_NAME}.tar.gz (${REMOTE_BYTES} bytes in s3://${SPACES_BUCKET}/sso-keycloak/)"
fi
...
echo "  aws s3 ls s3://${SPACES_BUCKET}/sso-keycloak/ --endpoint-url https://${SPACES_REGION}.digitaloceanspaces.com"
