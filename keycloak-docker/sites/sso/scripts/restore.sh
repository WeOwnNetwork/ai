...
#   - Local filename:  sso-keycloak_backup_20260115_120000
#   - S3 URL:          s3://bucket-name/sso-keycloak/sso-keycloak_backup_20260115_120000.tar.gz
set -euo pipefail
...
if [[ $# -eq 2 ]]; then
  ...
  echo ""
  echo "Examples:"
  echo "  $0 root@143.198.xxx.xxx sso-keycloak_backup_20260115_120000"
  echo "  $0 sso-keycloak_backup_20260115_120000"
  echo ""
  echo "Note: Run via Infisical on the droplet:"
  echo "  ssh root@<host> 'cd /opt/sso_keycloak && infisical run -- docker compose exec keycloak /opt/keycloak/restore.sh ...'"
  exit 1
fi
...
if [[ ! -f "\$BACKUP_FILE" && "$REMOTE_STORAGE" == "do-spaces" && -n "\${SPACES_ACCESS_KEY:-}" && -n "\${SPACES_SECRET_KEY:-}" ]]; then
  ...
  aws s3 cp "s3://${SPACES_BUCKET}/sso-keycloak/${BACKUP_NAME}.tar.gz" \
    "\$BACKUP_FILE" \
...
echo "==> Restoring sso-keycloak from \$BACKUP_NAME"
echo ""

# --- Stop keycloak to prevent writes during restore ---
echo "==> Stopping Keycloak..."
docker compose -f "\$APP_DIR/compose.yaml" stop keycloak
...
docker compose -f "\$APP_DIR/compose.yaml" exec -T db psql -U "\${POSTGRES_USER:-keycloak}" -d "\${POSTGRES_DB:-keycloak}" -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;" 2>/dev/null || true
docker compose -f "\$APP_DIR/compose.yaml" exec -T db psql -U "\${POSTGRES_USER:-keycloak}" -d "\${POSTGRES_DB:-keycloak}" < "\$WORK_DIR/db.sql"
echo "    Database restore complete"

# --- Restore Keycloak data volume ---
echo "==> Restoring Keycloak data volume..."
# Wipe via alpine (root on the volume) — the Keycloak image runs as a
# non-root user on hardened (Minimus) builds and cannot delete its own
# data dir (rm: Permission denied on data/tmp, seen live 2026-08-02).
docker run --rm \\
  -v "\${PROJECT_NAME}_keycloak_data:/data" \\
  alpine:3.19 \\
...
echo "    Keycloak data restore complete"
...
docker compose -f "\$APP_DIR/compose.yaml" start keycloak
...
echo ""
echo "Keycloak Admin Console: https://sso.weown.id/admin"
SCRIPT
