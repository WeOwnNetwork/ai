...
# --- Idempotency check ------------------------------------------------------
existing="$(curl -fsS -H "SIGNOZ-API-KEY: ${SIGNOZ_API_KEY}" "$RULES_URL" || true)"
...
http_code="$(curl -sS -o /tmp/signoz-oom-alert-resp.json -w '%{http_code}' \
  -X POST "$RULES_URL" \
...
if [[ "$http_code" =~ ^2 ]]; then
  ...
  cat /tmp/signoz-oom-alert-resp.json >&2 || true
  echo "" >&2
