...
#
#   ./allm-set-role.sh <ssh-target> <container> <username> <role> [--yes]
...
# No secret is read, written, or passed — the script touches the `role` column.
set -euo pipefail
...
ROLE="${4:?role required: admin | manager | default}"
YES="${5:-}"
case "$ROLE" in admin|manager|default) ;; *) echo "ERROR: role must be admin | manager | default (got '$ROLE')" >&2; exit 1 ;; esac
