#!/usr/bin/env bash
# beta-weown-chat — convenience wrapper for site operations
#
...
#   health [ip]              Check health endpoint (https://beta-chat.weown.dev/api/ping)
...
#   ./site.sh health                    # Check AnythingLLM health via HTTPS
...
cmd_restore() {
  ...
  if [[ -z "$arg1" ]]; then
    ...
    echo "Example: $0 restore beta-weown-chat_backup_20260606_120000" >&2
    exit 1
  fi
...
cmd_health() {
  local ip="${1:-$(get_droplet_ip)}"
  local url="https://beta-chat.weown.dev/api/ping"
  ...
  if curl -sf "$url" >/dev/null 2>&1; then
    ...
    echo "FAIL"
    echo "    Tried: $url" >&2
    echo "    Try SSH check: ssh root@$ip 'docker compose -f /opt/beta_weown_chat/compose.yaml ps'" >&2
    exit 1
  fi
}
...
cmd_help() {
  cat <<EOF
beta-weown-chat — site operations wrapper
...
Examples:
  ...
  $0 health                    # Check AnythingLLM health via HTTPS
  $0 smoke-test                # Run smoke test framework
