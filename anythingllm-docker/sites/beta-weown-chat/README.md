...
2. **First deploy needs `BAO_WRAP_TOKEN`** — a single-use, VPC-bound,
   ...
   leaves `role_id` unchanged, so this render stays valid. The entrypoint
   unwraps it on-box at first boot and blanks it; restarts reuse the unwrapped
   secret-id at `{{ app_dir }}/.bao-secret-id`.
