#!/usr/bin/env bash
# dev-weown-anythingllm-anythingllm — convenience wrapper for site operations
#
...
cmd_restore() {
  ...
  if [[ -z "$arg1" ]]; then
    ...
    echo "Example: $0 restore dev-weown-anythingllm-anythingllm_backup_20260606_120000" >&2
    exit 1
  fi
...
cmd_health() {
  ...
  if curl -sf "$url" >/dev/null 2>&1; then
    ...
    echo "FAIL"
    echo "    Tried: $url" >&2
    echo "    Try SSH check: ssh root@$ip 'docker compose -f /opt/dev_weown_anythingllm_anythingllm/compose.yaml ps'" >&2
    exit 1
  fi
}
...
cmd_help() {
  cat <<EOF
dev-weown-anythingllm-anythingllm — site operations wrapper
