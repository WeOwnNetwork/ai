#!/usr/bin/env bash
# dev-weown-anythingllm-anythingllm - Skinny Backup Script
...
BACKUP_NAME="dev-weown-anythingllm-anythingllm_backup_$TIMESTAMP"
...
docker run --rm \
  -v "dev_weown_anythingllm_anythingllm_storage:/data:ro" \
  -v "$WORK_DIR:/backup" \
  alpine:3.19 \
...
docker run --rm \
  -v "dev_weown_anythingllm_anythingllm_caddy_data:/data:ro" \
  -v "$WORK_DIR:/backup" \
  alpine:3.19 \
...
if [[ "$REMOTE_STORAGE" == "do-spaces" ]]; then
  ...
  echo "==> Uploading to DO Spaces (s3://${SPACES_BUCKET}/dev-weown-anythingllm-anythingllm/)..."
  export AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY"
  export AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY"
  aws s3 cp "$UPLOAD_FILE" \
    "s3://${SPACES_BUCKET}/dev-weown-anythingllm-anythingllm/" \
  ...
  REMOTE_BYTES=$(aws s3 ls "s3://${SPACES_BUCKET}/dev-weown-anythingllm-anythingllm/$(basename "$UPLOAD_FILE")" \
    --endpoint-url "https://${SPACES_REGION}.digitaloceanspaces.com" \
    | awk '{print $3}' | tail -1)
  ...
  echo "==> Remote backup verified: $(basename "$UPLOAD_FILE") (${REMOTE_BYTES} bytes in s3://${SPACES_BUCKET}/dev-weown-anythingllm-anythingllm/)"
...
echo "  aws s3 ls s3://${SPACES_BUCKET}/dev-weown-anythingllm-anythingllm/ --endpoint-url https://${SPACES_REGION}.digitaloceanspaces.com"
