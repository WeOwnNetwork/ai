...
Execute the INT-P01 (`ai.weown.agency`) DOKS → Docker migration end-to-end. The plan, ADR, and rendered site already exist in this branch — your job is the operational execution: local test, staging droplet, Jason review, production DNS cutover, DOKS decommission. **Do not modify the DOKS source environment until production cutover succeeds.**
