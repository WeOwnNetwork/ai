#!/usr/bin/env bash
# int-s004-anythingllm — convenience wrapper for site operations
#
...
#   health [ip]              Check AnythingLLM health endpoint
...
#   ./site.sh health                    # Check health endpoint
...
cmd_restore() {
  ...
  if [[ -z "$backup_name" ]]; then
    ...
    echo "Example: $0 restore int-s004-anythingllm_backup_20260606_120000" >&2
    exit 1
  fi
...
cmd_health() {
  local ip="${1:-$(get_droplet_ip)}"
  local url="http://$ip:3001/api/ping"
...
cmd_help() {
  cat <<EOF
int-s004-anythingllm — site operations wrapper
