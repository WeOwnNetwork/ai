...
- **`<dev>` → GitHub username mapping in `auto-pr-to-main.yml` step 6** — new inline `case` statement that maps short handles (`roman`, `nik`, `yonks`, `mohammed`, `shahid`, `dhruv`) to full GitHub usernames (`romandidomizio`, `ncimino`, `YonksTEAM`, `iamwaseem18`, `mshahid538`, `dhruvmalik007`) so the PR body's `Triggered by: @<username>` line pings the correct account rather than a non-existent `@roman` / `@nik`. Source of truth for the mapping is `CONTRIBUTING.md` §4 "Known contributor handles" table — the case statement must be kept in sync on onboarding / offboarding. Unknown handles fall through unchanged; reviewer catches during PR review. Resolves 4 of 6 Copilot review comments on PR #13 that flagged `@<short-handle>` as potential misattribution.
...
- **`CONTRIBUTING.md` §4** now explicitly documents that the `<dev>` segment **does not** need to equal your GitHub username (the auto-PR mapping handles that), removing a user-facing ambiguity flagged by Copilot review on PR #13.
