...
Continuation iteration of the in-flight PR #13 (originally tagged `[v3.3.4.2]` below). Bumped to `v3.3.5.1` because all changes in this section were made on 2026-04-27, which is the 5th week of April under the active #WeOwnVer interpretation (`vSEASON.MONTH.WEEK.ITERATION` → Season 3, April, Week 5, Iteration 1). The original 2026-04-23 work + Copilot rounds 1–3 (2026-04-24) remain under `[v3.3.4.2]` below; rounds 4–7 (all on 2026-04-27) are documented here. The PR itself remains a single squash-merge unit; the CHANGELOG split is purely about #WeOwnVer fidelity per the active versioning calendar.
...
- **PII minimization audit (round-6 + round-7 + round-8)**: zero email addresses anywhere in PR body output paths. Step 7 contributors fallback (`%an` only), step 8 Recent Commits header (`%an` only). Round-8 extension: all 3 `roman@weown.email` references in `workflows/README.md` (§2 Account Security Requirements, §7 Layer 1 alert recipient, §10 Transition Checklist) rewritten to generic internal-runbook language — personal emails are now ZERO across the entire in-PR file set. Aligns with prior PII minimization on `CONTRIBUTING.md` handle table + `CODEOWNERS`. Per GDPR Art. 5(1)(c) data minimization, ISO/IEC 27001:2022 A.5.34 (privacy + protection of PII). (Note: `roman@weown.email` still appears in out-of-scope files — `vaultwarden/README.md`, `vaultwarden/CHANGELOG.md`, `anythingllm/helm/Chart.yaml` — which are different business contexts and separate PRs; intentionally NOT modified to avoid expanding PR #13's scope.)
...
- **`.github/workflows/README.md` header** — version bumped from `v3.3.4.1` to `v3.3.4.2` per #WeOwnVer pre-merge iteration policy (this is the second merge-event iteration in the same ISO week). _A subsequent bump to `v3.3.5.1` was applied in the same PR on 2026-04-27 once the work crossed into ISO week 5 of April — see [`[v3.3.5.1]`](#v3351--2026-04-27) above._
...
- **Version cadence**: this section captures the Season 3, April Week 4 work (original PR + Copilot rounds 1–3). Subsequent same-PR work on 2026-04-27 crossed into Week 5 of April and is documented under [`[v3.3.5.1]`](#v3351--2026-04-27) above. See `docs/VERSIONING_WEOWNVER.md` for the full cadence rules.
