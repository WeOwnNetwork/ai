...
- **PII minimization audit (round-6 + round-7)**: zero email addresses anywhere in PR body output paths. Step 7 contributors fallback (`%an` only), step 8 Recent Commits header (`%an` only). Aligns with prior PII minimization on CONTRIBUTING.md handle table + CODEOWNERS. Per GDPR Art. 5(1)(c) data minimization, ISO/IEC 27001:2022 A.5.34 (privacy + protection of PII).
