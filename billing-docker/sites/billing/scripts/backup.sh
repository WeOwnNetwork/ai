#!/usr/bin/env bash
# weown-billing - Skinny Backup Script
...
BACKUP_NAME="weown-billing_backup_$TIMESTAMP"
...
docker exec -e PGPASSWORD="$POSTGRES_PASSWORD" "${PROJECT_NAME}-db-1" \
  pg_dump -U "${POSTGRES_USER:-billing}" -d "${POSTGRES_DB:-billing}" \
...
if [[ "$REMOTE_STORAGE" == "do-spaces" ]]; then
  ...
  echo "==> Uploading to DO Spaces (s3://${SPACES_BUCKET}/weown-billing/)..."
  export AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY"
  export AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY"
  aws s3 cp "$BACKUP_DIR/${BACKUP_NAME}.tar.gz" \
    "s3://${SPACES_BUCKET}/weown-billing/" \
  ...
  REMOTE_BYTES=$(aws s3 ls "s3://${SPACES_BUCKET}/weown-billing/${BACKUP_NAME}.tar.gz" \
    --endpoint-url "https://${SPACES_REGION}.digitaloceanspaces.com" \
    | awk '{print $3}' | tail -1)
  ...
  echo "==> Remote backup verified: ${BACKUP_NAME}.tar.gz (${REMOTE_BYTES} bytes in s3://${SPACES_BUCKET}/weown-billing/)"
fi
...
# --plain: token-in-env, no CLI keyring (bare login hangs headless)
...
echo "  aws s3 ls s3://${SPACES_BUCKET}/weown-billing/ --endpoint-url https://${SPACES_REGION}.digitaloceanspaces.com"
