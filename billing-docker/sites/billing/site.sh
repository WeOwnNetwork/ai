#!/usr/bin/env bash
# weown-billing — convenience wrapper for site operations
#
...
#   health [ip]              Check health endpoint (https://billing.weown.dev/healthz)
...
#   ./site.sh health                    # Check billing app health via HTTPS
...
cmd_restore() {
  ...
  if [[ -z "$arg1" ]]; then
    ...
    echo "Example: $0 restore weown-billing_backup_20260606_120000" >&2
    exit 1
  fi
...
cmd_health() {
  local ip="${1:-$(get_droplet_ip)}"
  local url="https://billing.weown.dev/healthz"
  ...
  if curl -sf "$url" >/dev/null 2>&1; then
    ...
    echo "FAIL"
    echo "    Tried: $url" >&2
    echo "    Try SSH check: ssh root@$ip 'docker compose -f /opt/weown_billing/compose.yaml ps'" >&2
    exit 1
  fi
}
...
cmd_help() {
  cat <<EOF
weown-billing — site operations wrapper
...
Examples:
  ...
  $0 health                    # Check billing app health via HTTPS
