...
3. **Interactive prompt** for one-shot admin scripts (rotation, breakglass,
   ...
   ```bash
   # Dual-shell on purpose: zsh reads via its "VAR?prompt" form; bash
   # rejects that operand as an identifier (error silenced) and falls
   # through the || to its own -p prompt form. Verified in both shells -
   # keep the two lines together, never copy just one.
