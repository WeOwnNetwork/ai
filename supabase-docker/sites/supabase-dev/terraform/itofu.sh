#!/usr/bin/env bash
# supabase-dev — itofu.sh
#
# Run OpenTofu with infra secrets injected from Infisical (no
# terraform.tfvars on disk, no manual paste).
#
# Secrets: all TF_VAR_* provisioning values live in the operator-only
# Infisical project `<bootstrap-project>` — NOT the app project the
# droplet's Machine Identity can read (<runtime-project>). This wrapper
# runs tofu under `infisical run`, so tofu reads them as TF_VAR_* env
# vars; `init` forwards the DO Spaces creds to the S3 backend (the
# backend block can't read TF_VAR_*).
#
...
#
# Required Infisical secrets (<bootstrap-project>, dev env):
...
#   export WEOWN_TOFU_PROJECT_ID=<<bootstrap-project> Infisical project id>
...
# operator-global (same across all sites for this service), not site-specific.
# Set it once in your shell profile or pass it as an env var.

set -euo pipefail

: "${WEOWN_TOFU_PROJECT_ID:?Set WEOWN_TOFU_PROJECT_ID to the <bootstrap-project> Infisical project id (operator-only infra secrets).}"
ENV_SLUG="${WEOWN_TOFU_ENV:-dev}"
...
# Run a command with the <bootstrap-project> secrets injected as env vars.
