#!/usr/bin/env bash
# claw-weown-tools - Restore Script
...
#   - Local filename:  claw-weown-tools_backup_20260115_120000
#   - S3 path:         s3://bucket-name/claw-weown-tools/claw-weown-tools_backup_20260115_120000.tar.gz
set -euo pipefail
...
if [[ $# -eq 2 ]]; then
  ...
  echo ""
  echo "Examples:"
  echo "  INFISICAL_PROJECT_ID=abc123 $0 root@198.51.100.42 claw-weown-tools_backup_20260115_120000"
  echo "  $0 claw-weown-tools_backup_20260115_120000"
  exit 1
fi
...
if [[ ! -f "\$BACKUP_FILE" && "$REMOTE_STORAGE" == "do-spaces" && -n "\${SPACES_ACCESS_KEY:-}" && -n "\${SPACES_SECRET_KEY:-}" ]]; then
  ...
  aws s3 cp "s3://${SPACES_BUCKET}/claw-weown-tools/${BACKUP_NAME}.tar.gz" \
    "\$BACKUP_FILE" \
...
echo "==> Restoring claw-weown-tools from \$BACKUP_NAME"
echo ""
...
docker run --rm \
  -v "claw_weown_tools_workspace:/data" \
  -v "\$WORK_DIR:/backup:ro" \
  alpine:3.19 \
...
if [[ -f "\$WORK_DIR/caddy_data.tar.gz" ]]; then
  ...
  docker run --rm \
    -v "claw_weown_tools_caddy_data:/data" \
    -v "\$WORK_DIR:/backup:ro" \
    alpine:3.19 \
...
echo ""
echo "OpenClaw URL: https://claw.weown.tools"
echo ""
...
SCRIPT
  ...
  if [[ -n "$host" ]]; then
    ...
    ssh "$host" \
      "INFISICAL_PROJECT_ID='$INFISICAL_PROJECT_ID' INFISICAL_ENV='$INFISICAL_ENV' PROJECT_NAME='claw_weown_tools' BACKUP_NAME='$BACKUP_NAME' bash -s" <<'EOF'
set -euo pipefail
