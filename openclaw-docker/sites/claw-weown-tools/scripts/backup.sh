#!/usr/bin/env bash
# claw-weown-tools - Skinny Backup Script
...
BACKUP_NAME="claw-weown-tools_backup_$TIMESTAMP"
...
docker run --rm \
  -v "claw_weown_tools_data:/data:ro" \
  -v "$WORK_DIR:/backup" \
  alpine:3.19 \
...
docker run --rm \
  -v "claw_weown_tools_workspace:/data:ro" \
  -v "$WORK_DIR:/backup" \
  alpine:3.19 \
...
docker run --rm \
  -v "claw_weown_tools_caddy_data:/data:ro" \
  -v "$WORK_DIR:/backup" \
  alpine:3.19 \
...
if [[ "$REMOTE_STORAGE" == "do-spaces" ]]; then
  ...
  echo "==> Uploading ($(basename "$BACKUP_DIR/${BACKUP_NAME}.tar.gz")) to DO Spaces (s3://${SPACES_BUCKET}/claw-weown-tools/)..."
  export AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY"
  export AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY"
  aws s3 cp "$BACKUP_DIR/${BACKUP_NAME}.tar.gz" \
    "s3://${SPACES_BUCKET}/claw-weown-tools/" \
  ...
  REMOTE_BYTES=$(aws s3 ls "s3://${SPACES_BUCKET}/claw-weown-tools/$(basename "$BACKUP_DIR/${BACKUP_NAME}.tar.gz")" \
    --endpoint-url "https://${SPACES_REGION}.digitaloceanspaces.com" \
    | awk '{print $3}' | tail -1)
  ...
  echo "==> Remote backup verified: $(basename "$BACKUP_DIR/${BACKUP_NAME}.tar.gz") (${REMOTE_BYTES} bytes in s3://${SPACES_BUCKET}/claw-weown-tools/)"
fi
...
echo "  aws s3 ls s3://${SPACES_BUCKET}/claw-weown-tools/ --endpoint-url https://${SPACES_REGION}.digitaloceanspaces.com"
