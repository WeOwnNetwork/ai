#!/usr/bin/env bash
# claw-weown-dev — convenience wrapper for site operations
#
...
#   health [ip]              Check health endpoint (https://claw.weown.dev/)
...
#   ./site.sh health                    # Check OpenClaw health via HTTPS
...
cmd_restore() {
  ...
  if [[ -z "$arg1" ]]; then
    ...
    echo "Example: $0 restore claw-weown-dev_backup_20260606_120000" >&2
    exit 1
  fi
...
cmd_health() {
  local ip="${1:-$(get_droplet_ip)}"
  local url="https://claw.weown.dev/"
  ...
  if curl -sf "$url" >/dev/null 2>&1; then
    ...
    echo "FAIL"
    echo "    Tried: $url" >&2
    echo "    Try SSH check: ssh root@$ip 'docker compose -f /opt/claw_weown_dev/compose.yaml ps'" >&2
    exit 1
  fi
}
...
cmd_help() {
  cat <<EOF
claw-weown-dev — site operations wrapper
...
Examples:
  ...
  $0 health                    # Check OpenClaw health via HTTPS
